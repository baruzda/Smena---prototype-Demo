...
| `APPROVAL_ERROR` | Provisional без вопроса; approved с blocking-вопросом; active exception над provisional base rule | `provisional-without-question`, `approved-with-blocking-question`, `active-exception-on-provisional-rule` | covered |
...
| `MIGRATION_GAP` | Deprecated до verification; verified без evidence; bound current template без surface | `migration-deprecated-before-verified`, `migration-verified-without-evidence`, `bound-template-without-surface` | covered |
...
`test:card-rules` запускает 6 valid и 20 invalid isolated fixtures. `test:card-rules-validator` сохраняет исходный smoke test: два запуска основного генератора должны иметь одинаковый результат.
