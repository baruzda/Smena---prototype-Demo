...
## DEC-015 — утверждённый контракт ошибок каталога

Контекст: `docs/catalog-rules-matrix.md` уже требует отличать ошибку загрузки от пустой выдачи и сохранять старые данные при неудачном обновлении. Пользователь поручил завершить миграцию всех связанных UI-состояний. Решение: `catalog.error` используется только без доступных данных, заменяет контент ленты, сообщает «не удалось обновить каталог» и предлагает «повторить»; `catalog.stale` показывается inline над сохранёнными карточками, сообщает, что данные неактуальны, и предлагает «обновить». Дата, фильтры и настройки остаются видимыми. Для воспроизводимой проверки прототип принимает `?catalogState=error|stale`; успешное действие очищает параметр и возвращает ready-state. Последствие: оба состояния получают runtime, resolver, E2E и visual evidence и переводятся в `verified`. Не входит: реальная сеть, cache TTL и retry policy production-клиента.
